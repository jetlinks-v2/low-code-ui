...
<template>
  ...
  >
    <template v-for="item in columns" #[item.dataIndex]="{ record, index, valueChange }">
      <FormItem v-model:modelValue="record[item.dataIndex]"
        @change="() => { valueChange(record[item.dataIndex]) }" :item-type="item.type" :disabled="item.componentProps?.disabled" :component-props="item.componentProps" />
    </template>
  </QuickEditTable>
<!--  <j-table-->
<!--    bordered-->
