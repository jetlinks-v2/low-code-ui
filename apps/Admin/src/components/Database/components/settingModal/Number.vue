<template>
  ...
  <j-form-item v-if="model.validator.provider === 'range'" label="范围" style="margin-bottom: 0">
    <div class="setting-range">
      <j-form-item style="flex: 1" :name="['validator', 'configuration', 'min']" :rules="rules.min">
        <j-input-number v-model:value="model.validator.configuration.min" style="width: 100%;" />
      </j-form-item>
      <div style="margin-top: 4px">-</div>
      <j-form-item style="flex: 1" :name="['validator', 'configuration', 'max']" :rules="rules.max">
        <j-input-number v-model:value="model.validator.configuration.max" style="width: 100%;" />
      </j-form-item>
    </div>
  </j-form-item>
  <j-form-item v-if="model.validator.provider === 'max'" label="最大值" :name="['validator', 'configuration', 'value']">
    <j-input-number v-model:value="model.validator.configuration.value" style="width: 100%;" />
  </j-form-item>
  <j-form-item v-if="model.validator.provider === 'min'" label="最小值" :name="['validator', 'configuration', 'value']">
    <j-input-number v-model:value="model.validator.configuration.value" style="width: 100%;" />
  </j-form-item>
