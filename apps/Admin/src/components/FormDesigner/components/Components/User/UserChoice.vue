<template>
    <j-modal visible title="用户选择" :width="1000" @cancel="close" @ok="submitData">
        ...
        <div style="display: flex;margin-top: 20px;">
            <div class="leftTree">
                <div style="display: flex">
                    <j-select v-if="sourceType === 'role'"
                        :options="[{ label: '角色名', value: 'name' }, { label: '分组', value: 'group' }]" style="min-width: 90px"
                        v-model:value="searchType"></j-select>
                    <j-input v-model:value="searchValue" :placeholder="sourceType === 'tissue' ? '请输入组织名称' : '请输入角色'"
                        class="search-input" allowClear @pressEnter="search">
                        <template #suffix>
                ...
                <div style="margin-top: 10px;">
                    ...
                    <j-tree v-if="treeData.length > 0" :tree-data="treeData" @select="selectGroup"
                        v-model:selectedKeys="selectedKeys" v-model:expandedKeys="expandedKeys"></j-tree>
            ...
            <div class="rightTable">
                ...
                <j-pro-table model="TABLE" :columns="columns" :params="params" :defaultParams="{
                    sorts: [{ name: 'createTime', order: 'desc' }],
                }" 
                    :request="queryUser" ref='tableRef' class="table" :rowSelection="{
...
const queryTree = (searchValue?: any) => {
    ...
    if (sourceType.value === 'tissue') {
        ...
        getRoleList(params,searchType.value === 'name' ? true : false).then((res: any) => {
            if (res.status === 200 && res.result) {
                treeData.value = dealTreeData(res.result)
        ...
        const res = await getUserList({
            ...
            terms: [
                ...params.terms,
                {
                    terms: [
                        {
                            column: sourceType.value === 'tissue' ? 'id$in-dimension$org$not' : 'id$in-dimension$role',
